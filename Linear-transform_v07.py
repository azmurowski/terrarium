    ...
    def update(self,canvas_instance):
        ...
        for i in range(1, len(self.points)):
            if i != 1:
                ...
                canvas_instance.create_line(x,y,x1,y1,width = self.lineWidth, fill = self.lineColour, smooth=True)
        if i > 1:
            x = RealToScreenX(self.points[i][0], self.globalScale)
            y = RealToScreenY(self.points[i][1], self.globalScale)
            x0 = RealToScreenX(self.points[1][0], self.globalScale)
            y0 = RealToScreenY(self.points[1][1], self.globalScale)
            canvas_instance.create_line(x,y,x0,y0,width = self.lineWidth, fill = self.lineColour, smooth=True)
        ...
        for i in range(1, len(self.transformed)):
            if i != 1:
                ...
                canvas_instance.create_line(x, y, x1, y1, width=self.transformedLineWidth , fill=self.transformedLineColour, smooth=True)
        if i > 1:
            x = RealToScreenX(self.transformed[i][0], self.globalScale)
            y = RealToScreenY(self.transformed[i][1], self.globalScale)
            x0 = RealToScreenX(self.transformed[1][0], self.globalScale)
            y0 = RealToScreenY(self.transformed[1][1], self.globalScale)
            canvas_instance.create_line(x, y, x0, y0, width=self.transformedLineWidth , fill=self.transformedLineColour, smooth=True)
                                         # draws line from last point to first point
        ...
        for i in range(1, len(self.points)):
            ...
            canvas_instance.create_rectangle(x - self.pointRadius, y - self.pointRadius, x + self.pointRadius, y + self.pointRadius, fill=self.pointFill)

            label = "(" + str(round(self.points[i][0],2)) + "," + str(round(self.points[i][1],2)) + ")"
            canvas_instance.create_text(x, y - (15), text=label,font = ("Roboto", int(8)))

        for i in range(1, len(self.transformed)):
            x = RealToScreenX(self.transformed[i][0], self.globalScale)
            y = RealToScreenY(self.transformed[i][1], self.globalScale)

            canvas_instance.create_rectangle(x - self.pointRadius, y - self.pointRadius, x + self.pointRadius,
                                        y + self.pointRadius, fill=self.pointFill, outline=self.transformedFill)

            xLabel = round(self.transformed[i][0], 2)
            xLabel = str(xLabel)
            yLabel = round(self.transformed[i][1], 2)
            yLabel = str(yLabel)
            label = "(" + xLabel + "," + yLabel + ")"
        ...
        #draws cross
        canvas_instance.create_line(x - 5, y + 5, x +5, y -5, fill=self.originFill, width=3, capstyle=ROUND)
        canvas_instance.create_line(x - 5, y - 5, x + 5, y + 5, fill=self.originFill, width=3, capstyle= ROUND)
